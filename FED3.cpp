...
void FED3::leftStimulus() {
  ...
  strip.setPixelColor(2, strip.Color(2, 255, 2, 2) ); // 2 here is the 0th light index
  strip.show();
  strip.clear();
}
...
void FED3::rightStimulus() {
  digitalWrite (MOTOR_ENABLE, HIGH);
  strip.setPixelColor(10, strip.Color(255, 0, 255, 255) ); // 9 here is the 7th light index
  strip.show();
  strip.clear();
}
...
void FED3::logdata() {
  ...
  // INCLUDE CONDITION TO CATCH NO POKES
  else if (Event == "Left light" || Event == "Right light")
  {
    logfile.println(sqrt (-1)); // print NaN
  }
